...
// conversation partner, the unit tests, dialogues, grammar practice, the
// writing coach.
//
  ...
  wrap.appendChild(
    el("div", { class: "co-fix" }, [
      ...
      el("div", { class: "co-line co-right" }, [
        ...
        el("button", {
          class: "btn btn-sm", style: "margin-left:.4rem",
          onclick: (e) => { e.stopPropagation(); audioEngine.speak(corrected); }
        }, "🔊")
      ])
    ])
  );
  ...
  hits.forEach((h) => {
    ...
    if (!compact) {
      ...
      if (h.examples.length) {
        item.appendChild(
          el("div", { class: "co-examples" }, h.examples.slice(0, 3).map((ex) =>
...
// True when the text has something worth correcting — for callers that want to
// decide before building UI.
