...
export default function MapPage() {
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      ...
      {/*
        In production this section is a Mapbox GL canvas with custom growth-stage
        markers (see docs/05-tokyo-impact-map.md). This scaffold renders the same
        underlying data as an accessible station grid — the documented no-JS/no-map
        fallback — so the page is fully functional without a Mapbox token.
      */}
        ...
        {stations.map((s) => (
          <Card key={s.slug} className="p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-y-2 text-xs text-forest-900/70">
              ...
              <span>Active seeds: {s.activeSeedCount}</span>
            ...
            <div className="mt-4 flex items-center justify-between text-xs">
              ...
              <span className="font-display font-semibold text-forest-900">{s.growthScore}/100</span>
            </div>
            ...
            {s.growthStage === "bare_soil" && (
              <p className="mt-3 text-xs font-medium text-earth-600">
                Needs its first seeds — be the first to plant one here.
              </p>
            )}
          </Card>
        ))}
