// SM-2 inspired spaced repetition engine with a fixed interval ladder
// matching the spec: same day, 1, 3, 7, 14, 30, 90, 180 days.

import { store } from "./storage.js";

const INTERVAL_STEPS = [0, 1, 3, 7, 14, 30, 90, 180]; // days
const MIN_EASE = 1.3;
const DEFAULT_EASE = 2.5;

// quality: 0 = Again (total blank), 1 = Hard-fail, 2 = Hard, 3 = Good, 4 = Easy, 5 = Very easy
export function gradeItem(itemId, type, quality) {
  ...
  if (!item) {
    item = {
      id: itemId,
      type, // "vocab" | "grammar" | "listening" | "speaking" | "dialogue" | "reading"
      repetition: 0,
...
export function masteryLevel(itemId) {
  const item = store.state.srs[itemId];
  if (!item) return 0;
  // 0-100 scale derived from repetition depth + accuracy
