...
export function downloadBackup() {
  ...
  a.href = url;
  a.download = `vamos-progreso-${todayISO()}.json`;
...
export function makeCode() {
  const code = "VAMOS1:" + utf8ToB64(JSON.stringify(JSON.parse(snapshot())));
  markSaved();
  return code;
}
...
export function restoreFromCode(raw) {
  ...
  if (!text.startsWith("VAMOS1:")) {
    return { ok: false, error: "That doesn't look like a backup code — it should start with VAMOS1:" };
  }
  let json;
  try {
    json = b64ToUtf8(text.slice("VAMOS1:".length));
  } catch (e) {
