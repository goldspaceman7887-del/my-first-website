...
export function renderDialogueList(container) {
  container.appendChild(
    el("div", { class: "page-header" }, [
      el("h1", {}, "💬 Dialogues"),
      el("p", {}, "10 realistic beginner conversations across daily-life situations — listen, read along, and check comprehension.")
    ])
  );
...
export function renderDialogueDetail(container, params) {
    ...
    script.appendChild(
      el("div", { class: "dialogue-line" }, [
        ...
        el("div", { class: "line-content" }, [
          el("div", { class: "line-zh hanzi" }, l.zh),
          el("div", { class: "line-py" }, l.py),
          el("div", { class: "line-en" }, l.en)
        ]),
        el("div", { class: "line-controls" }, [el("button", { class: "play-btn", onclick: () => audioEngine.speak(l.zh) }, "🔊")])
      ])
    );
  });
  ...
  function playAll(rate) {
    ...
    const step = () => {
      ...
      audioEngine.speak(d.lines[i].zh, { rate, onend: () => { i++; setTimeout(step, 250); } });
    };
    step();
  }
  ...
  container.appendChild(
    el("div", { class: "flex gap-2 flex-wrap" }, d.vocabHighlights.map((v) => el("span", { class: "badge badge-default" }, `${v.w} ${v.py} — ${v.en}`)))
  );

  if (d.culturalNote) {
    container.appendChild(
      el("div", { class: "culture-note-block", style: "margin-top:1rem" }, [el("h4", {}, "Cultural note"), el("p", {}, d.culturalNote)])
    );
  }
  ...
  function finishQuiz() {
    ...
    if (wasNew) {
      ...
      addXP(xp, `Dialogue: ${d.title}`);
      xpToast(xp, d.title);
      store.save();
    }
    toast(`Comprehension: ${correctCount}/${d.comprehension.length} correct`, { icon: "✅" });
  }
}
