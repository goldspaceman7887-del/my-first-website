// STORY MODE — graded mini-stories built from taught vocabulary, shown
// paragraph by paragraph (Chinese / pinyin / English), followed by
// vocabulary review, comprehension questions, and speaking questions.
...
export function renderStory(container) {
  container.appendChild(
    el("div", { class: "page-header" }, [
      el("h1", {}, "📖 Story Mode"),
      el("p", {}, "Short stories built almost entirely from vocabulary you've already met — read for meaning first, then review.")
    ])
  );
    ...
    STORIES.forEach((s) => {
      ...
      grid.appendChild(
        el("div", { class: "card", style: "cursor:pointer", onclick: () => showStory(s) }, [
          ...
          el("p", { class: "text-muted" }, s.titleZh)
        ])
      );
    });
  ...
  function showStory(s) {
    ...
    body.appendChild(el("h2", { style: "margin-top:.75rem" }, `${s.title} · ${s.titleZh}`));

    s.paragraphs.forEach((p) => {
      body.appendChild(
        el("div", { class: "card", style: "margin-top:.6rem" }, [
          el("div", { class: "flex justify-between items-center" }, [
            el("div", { class: "hanzi", style: "font-size:1.1rem" }, p.zh),
            el("button", { class: "play-btn", style: "width:34px;height:34px", onclick: () => audioEngine.speak(p.zh) }, "🔊")
          ]),
          el("div", { class: "pinyin" }, p.py),
    ...
    body.appendChild(
      el("div", { class: "flex gap-2 flex-wrap" }, s.vocabReview.map((v) => el("span", { class: "badge badge-default" }, `${v.w} ${v.py} — ${v.en}`)))
    );
    ...
    s.comprehension.forEach((c) => {
      ...
      compCard.appendChild(
        el("div", { style: "margin-bottom:.6rem" }, [
          ...
          el("button", { class: "btn btn-sm", onclick: (e) => { ans.classList.toggle("hidden"); } }, "Reveal answer"),
          ans
        ])
      );
    });
